...
class BLIP_BEV_VQA(nn.Module):
    ...
    def forward(self, bev, question, answer, det=None):
        ...
        '''
        answer = self.tokenizer(answer, padding='longest', truncation=True, max_length=300, return_tensors="pt").to(self.device)
    ...
    def generate(self, bev, question, max_length=300, min_length=1, top_p=0.9, temperature=0.8, det=None):
        ...
        question = self.tokenizer(question, padding='longest', truncation=True, max_length=300, return_tensors="pt").to(
            self.device)
        question.input_ids[:, 0] = self.tokenizer.enc_token_id
